...
object MatcherError {
  ...
  case class UnexpectedMatcherPublicKey(required: PublicKeyAccount, given: PublicKeyAccount)
      extends MatcherError(
        ...
        pubKey,
        unexpected,
        e"The require matcher public key for this DEX is ${'required -> Base58.encode(required.publicKey)}, but ${'given -> Base58.encode(given.publicKey)}"
      )
  case class UnexpectedSender(required: Address, given: Address)
      extends MatcherError(order, pubKey, unexpected, e"The sender ${'expected -> given} does not match expected ${'required -> required}")
  ...
  case class FeeNotEnough(required: Long, given: Long, assetId: Option[AssetId])
      extends MatcherError(
        ...
        fee,
        notEnough,
        e"DEX requires ${'required                                        -> required} ${'assetId -> AssetPair
          .assetIdStr(assetId)} as fee for this order, but given ${'given -> given} ${'assetId    -> AssetPair.assetIdStr(assetId)}"
      )
  ...
  case class AmountAssetIsBlacklisted(assetId: AssetId)
      extends MatcherError(asset, amount, blacklisted, e"The amount asset ${'assetId -> assetId} is blacklisted")
  case class PriceAssetIsBlacklisted(assetId: AssetId)
  ...
  case class OrderIsFull(id: Order.Id)      extends MatcherError(order, commonEntity, limitReached, e"The order ${'id -> id} is filled")
  case class OrderIsFinalized(id: Order.Id) extends MatcherError(order, commonEntity, immutable, e"The order ${'id    -> id} is finalized")
