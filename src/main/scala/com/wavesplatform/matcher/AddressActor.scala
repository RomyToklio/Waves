...
class AddressActor(
        ...
        .fold(activeOrders.get(id) match {
          ...
          case None =>
            val reason = orderDB.status(id) match {
              ...
              case OrderStatus.Filled(_)    => MatcherError.OrderIsFull(id)
            }
  ...
  private def handleOrderTerminated(lo: LimitOrder, status: OrderStatus.Final): Unit = {
    ...
    orderDB.saveOrder(lo.order)
    pendingCancellation.remove(lo.order.id()).foreach(_.success(api.OrderCancelRejected(MatcherError.OrderIsFinalized(lo.order.id()))))
